...
const Dashboard = () => {
  ...
  const [cumulativeLossArr, setCumulativeLossArr] = useState([]); // cumulative probability loss [nSteps]
  ...
  const handleRunSimulation = async () => {
    ...
    try {
      const { zArr, tArr, probArr, currentArr, cumulativeLossArr } = simulateN({
      ...
      setCumulativeLossArr(cumulativeLossArr);
      setTIndex(0);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-1/5 min-w-[300px] border-r border-gray-300 p-4 overflow-y-auto">
          <div className="space-y-6">
            <div>
              <h1 className="text-2xl font-bold mb-2">Simulator Test</h1>
          ...
          <div className="space-y-4">
            ...
            <div className="mt-4">
              ...
              <p>
                x &lt; 0 (Transmitted):{" "}
                {(() => {
                  if (!probArr || zArray.length < 2 || !cumulativeLossArr) return 0;
                  ...
                  const cumulativeLoss = cumulativeLossArr[tIndex] || 0;
                  const total = leftSum + rightSum + cumulativeLoss;
                  // Here we assume that all loss is attributed to the reflected side:
                  const reflected = leftSum + cumulativeLoss;
                  return total ? (reflected / total * 100).toFixed(2) : 0;
                })()}
                %
              </p>
              <p>
              ...
              <p>
                ...
                {(() => {
                  ...
                  const cumulativeLoss = cumulativeLossArr[tIndex] || 0;
                  const total = leftSum + rightSum + cumulativeLoss;
                  return total ? (cumulativeLoss / total * 100).toFixed(2) : 0;
                })()}
                %
              </p>
            </div>
